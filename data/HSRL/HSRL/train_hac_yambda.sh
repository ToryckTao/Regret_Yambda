...
# Check if environment model log exists (the .env file is just a placeholder, we use .model.log)
...
# Create symlink to use existing Yambda environment model
rm -rf output/yambda_hac/env/log
mkdir -p output/yambda_hac/env
ln -sf ../../yambda_hsrl/env/log output/yambda_hac/env/log
ln -sf ../yambda_hsrl/env/yambda_user_env_lr0.001_reg0.0001.env output/yambda_hac/env/yambda_user_env_lr0.001_reg0.0001.env 2>/dev/null || true
