...
mkdir -p output/yambda_hsrl/agents/

output_path="output/yambda_hsrl/"
log_name="yambda_user_env_lr0.001_reg0.0001"

# Check if environment model exists before running
if [ ! -f "output/yambda_hsrl/env/${log_name}.env" ]; then
    echo "Error: Environment model not found: output/yambda_hsrl/env/${log_name}.env"
    echo "Please run train_env.py or run_yambda_env.sh first."
    exit 1
fi
