#!/usr/bin/env python3
"""
Yambda Training Log Analyzer - Academic Publication Quality Visualization
Parses Yambda (YAMBA) RL training logs and generates training curves and evaluation dashboards.
"""
...
def parse_train_log(filepath: str) -> Dict[str, Any]:
    """
    Parse Yambda training log file.
        ...
        if avg_reward_match and avg_n_step_match:
            ...
            if avg_reward > 0:  # Filter out initial step with 0 rewards
                step_data.append({
                    ...
                    'critic_loss': float(critic_loss_match.group(1)) if critic_loss_match else np.nan,
                    'actor_loss': float(actor_loss_match.group(1)) if actor_loss_match else np.nan,
                    'behavior_loss': float(behavior_loss_match.group(1)) if behavior_loss_match else np.nan,
                })
    ...
    if not df.empty:
        ...
        summary['mean_critic_loss'] = df['critic_loss'].mean()
        summary['mean_actor_loss'] = df['actor_loss'].mean()
...
def plot_training_curves(data: Dict, output_path: str = 'yambda_training_curves.png'):
    """
    Generate training curve visualization for Yambda.
    ...
    # Actor loss (scale to show on same plot)
    ...
    # ========== Global Title ==========
    agent_name = hp.get('agent_class', 'Yambda')
    iter_count = hp.get('n_iter', 'N/A')
    fig.suptitle(f'Yambda ({agent_name}) Training Analysis\n'
                 f'Iterations: {iter_count}, Actor LR: {hp.get("actor_lr", "N/A")}, '
                 f'Critic LR: {hp.get("critic_lr", "N/A")}',
...
def plot_evaluation_summary(data: Dict, output_path: str = 'yambda_eval_summary.png'):
    """
    Generate evaluation summary visualization for Yambda.
    ...
    table_data = [
        ...
        ['N Iterations', format_hp(hp.get('n_iter'))],
    ]
    ...
    table[(7, 1)].set_text_props(fontweight='bold', color='#FFC107')
    
    ax2.set_title('(b) Evaluation Summary', pad=20, fontweight='bold', fontsize=12)
    
    # ========== Global Title ==========
    fig.suptitle(f'Yambda Training Evaluation Summary',
                 fontsize=16, fontweight='bold', y=1.02)
...
def plot_convergence_analysis(data: Dict, output_path: str = 'yambda_convergence.png'):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Parse Yambda training logs and generate visualization'
    )
    parser.add_argument('--log', type=str, required=True,
                        help='Path to Yambda training log file')
    parser.add_argument('--output_dir', type=str, default='output/yambda_hsrl/analysis',
