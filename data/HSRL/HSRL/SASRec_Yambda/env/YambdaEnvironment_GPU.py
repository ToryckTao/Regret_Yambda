...
class YambdaEnvironment_GPU(BaseRLEnvironment):
    ...
    def __init__(self, args):
        # 临时修改 reward_func，避免父类 eval 不存在的函数
        # Yambda 在 step 函数中自己处理 direct_score
        original_reward_func = args.reward_func
        if args.reward_func == 'direct_score':
            args.reward_func = 'mean_with_cost'  # 用一个存在的函数来绕过父类检查
        
        super(YambdaEnvironment_GPU, self).__init__(args)
        self.temper_sweet_point = args.temper_sweet_point
        self.temper_prob_lag = args.temper_prob_lag

        # 保存 reward_func 名称，用于区分处理逻辑
        self.reward_func_name = original_reward_func

        infile = open(args.urm_log_path, 'r')
        class_args = eval(infile.readline()) # example: Namespace(model='YambdaUserResponse', reader='RL4RSDataReader')
        model_args = eval(infile.readline()) # model parameters in Namespace
        print("Environment arguments: \n" + str(model_args))
        infile.close()
        print("Loading raw data")
        # 动态选择 Reader 类型，支持 YambdaDataReader 和 RL4RSDataReader
        if class_args.reader == 'YambdaDataReader':
            ...
            raise ValueError(f"Unsupported reader: {class_args.reader}. Expected YambdaDataReader or RL4RSDataReader")
        print(f"Using reader: {class_args.reader}")
    ...
    def step(self, step_dict):
        ...
        batch_data = {
            ...
            'history': self.current_observation['history'],  # 添加 history
        ...
        with torch.no_grad():
            ...
            # YambdaUserResponse 输出的是连续标量 preds (B, L)
            preds = output_dict['preds']

            # 判断奖励函数类型
            if self.reward_func_name == 'direct_score':
                # direct_score: 直接使用模型预测的连续标量作为奖励
                # preds shape: (B, L), 对每个 slate 取平均得到 (B,)
                immediate_reward = torch.mean(preds, dim=-1).detach()
                # 对于 history 更新，也需要转换为二值反馈
                response = (preds > 0).float()  # (B, L)
            else:
                # 原来的逻辑：基于点击反馈计算奖励
                # 将连续标量转换为二值反馈（用于原有 reward_func）
                response = (preds > 0).float()  # (B, L)
                # reward (B,)
            ...
            # Yambda 极简历史更新：不管用户反馈好坏，纯滑动窗口向前滚动
            # H_prime 维度是 (B, H+slate_size)，我们直接截取最新的 max_seq_len 个物品
