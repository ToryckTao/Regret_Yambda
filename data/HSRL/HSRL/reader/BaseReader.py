...
class BaseReader(Dataset):
    ...
    def _read_data(self, args):
        ...
        # 读取数据用于 RL 训练：train 最多 20 万，val/test 各 5 万
        self.data['train'] = pd.read_table(args.train_file, sep = args.data_separator, engine = 'python', nrows=200000)
        self.data['val'] = pd.read_table(args.val_file, sep = args.data_separator, engine = 'python', nrows=50000) \
                                if len(args.val_file) > 0 else self.data['train']
        self.data['test'] = pd.read_table(args.test_file, sep = args.data_separator, engine = 'python', nrows=50000) \
